...
def save_data_as_h5(iteration_count, data, is_next_state=False):
    with h5py.File('space_invaders/env_data/'+generate_file_name(iteration_count, is_next_state)+'.h5', 'w') as f:
...
def load_data_from_h5(iteration_count, is_next_state=False):
    data = None
    with h5py.File('space_invaders/env_data/'+generate_file_name(iteration_count, is_next_state)+'.h5', 'r') as f:
